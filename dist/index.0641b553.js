...
calculateBtnEl.addEventListener("click", ()=>{
    totalOutputMathEl.innerHTML = calculate(Number(veneerThickEl.value), Number(rollThickEl.value));
    totalOutputTabEl.innerHTML = calculate2(Number(veneerThickEl.value), Number(rollThickEl.value));
    rollThickEl.value = "";
});
// ### CALCULATE
const calculate = (veneerThick, rollThick)=>{
    ...
    let total1 = 0;
    for(i = INSIDE_ROLL_RADIUS; i <= INSIDE_ROLL_RADIUS + rollThick; i += veneerThick / 10)total1 = total1 + 2 * Math.PI * i;
    return `${Math.round(total1 / 100)} mb`;
};
...
},{}]},["lKzq4","bNKaB"], "bNKaB", "parcelRequirea142")
